...
#if !defined(_FULLSCREENEAGLE_H_)
#define _FULLSCREENEAGLE_H_
...
class CFullScreenDirectXEagle : public CLynxRender
{
    private:
    ...
    public:
		...
		virtual ~CFullScreenDirectXEagle()
		{
			mInitOK=false;
		}
        ...
        bool Create(CWnd *pcwnd,int src_width, int src_height, int scrn_width, int scrn_height, int zoom)
		{
			...
			try 
			{
				...
				intDesc.dwWidth  = 2*mSrcWidth;
				intDesc.dwHeight = 2*mSrcHeight;
