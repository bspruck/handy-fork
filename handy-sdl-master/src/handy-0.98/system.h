...
#else
#define HANDY_AUDIO_BUFFER_SIZE					4*(HANDY_AUDIO_SAMPLE_FREQ/4)
#endif
...
// Mikey system interfacing
   ...
   {
      mMikie->DisplaySetAttributes(Rotate,Format,Pitch,DisplayCallback,objref);
   };
...
// Miscellaneous
   ...
   void	SetButtonData(ULONG data)
   {
      mSusie->SetButtonData(data);
   };
   ULONG	GetButtonData(void)
   {
