...
#define NUMOFVIDEOMODES 3
      struct {
         uint32 x;
         uint32 y;
      } VModes[NUMOFVIDEOMODES] = { {320, 240}, {400, 240}, {480, 272} };
...
inline void handy_sdl_draw_filter(int filtertype, SDL_Surface *src, SDL_Surface *dst, Uint8 *delta)
{
   ...
   switch( filter ) {
      case 0:
         break;
      case 1:
         TVMode((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 2:
         _2xSaI((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 3:
         Super2xSaI((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 4:
         SuperEagle((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 5:
         MotionBlur((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 6:
         Simple2x((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 7:
         bilinear((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 8:
         bilinearPlus((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 9:
         Pixelate((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
      case 10:
         Average((Uint8 *)src->pixels, src->pitch, delta, dst_offset, dst->pitch, src->w, src->h);
         break;
   }
#else
   switch( filter ) {
...
#undef AVERAGE
      ...
      Eh += 102;
      if(Eh >= 240) {
         Eh -= 240;   // 102 - real atari lynx y size
         dh++;
      }
   }
}
...
#undef AVERAGE
      ...
      Eh += 102;
      if(Eh >= 272) {
         Eh -= 272;   // 102 - real atari lynx y size
         dh++;
      }
   }
}
