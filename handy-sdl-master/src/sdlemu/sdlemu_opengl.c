...
void sdlemu_init_opengl(SDL_Surface * src, SDL_Surface * dst, int texturetype, int filter, int src_bpp)
{
   ...
   if ( value )
      printf("OpenGL - Doublebuffer       : Enabled\n");
   else
      printf("OpenGL - Doublebuffer       : Disabled\n");

   SDL_GL_GetAttribute(SDL_GL_SWAP_CONTROL, &value);
   if ( value )
      printf("OpenGL - VSync              : Enabled\n");
   else
      printf("OpenGL - VSync              : Disabled\n");

   SDL_GL_GetAttribute(SDL_GL_ACCELERATED_VISUAL, &value);
   if ( value )
      printf("OpenGL - Acceleration       : Enabled\n");
   else
...
void sdlemu_draw_texture(SDL_Surface * src, SDL_Surface * dst, int texturetype)
{
   /*
...
//	SDL_BlitSurface(src, NULL, overlay, NULL);
   ...
   switch (texturetype) {
      ...
      default:
         ...
         glVertex3i(0, 0, 0);
         glTexCoord2f(texcoord[2], texcoord[1]);
         glVertex3i(dst->w, 0, 0);
         glTexCoord2f(texcoord[0], texcoord[3]);
         glVertex3i(0, dst->h, 0);
         glTexCoord2f(texcoord[2], texcoord[3]);
         glVertex3i(dst->w, dst->h, 0);
         glEnd();
   }//*/
...
//	glBlendFunc(GL_ONE, GL_ONE);
   ...
   if (filter) {
      ...
      printf("OpenGL - Filters            : Eisabled\n");
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
   }
